...
- **Single-select no longer deselects on re-click.** Clicking (or pressing Enter on) the
  already-selected option in a single-select (`multiple="false"`) silently emptied the control —
  surprising because a single-select row has no checkbox to signal "you're un-picking this". A
  re-click is now a no-op confirm that just closes the dropdown; the value stays put. Clearing is
